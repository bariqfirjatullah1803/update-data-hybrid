...
def check(email):
    url = "https://api.karismagarudamulia.com/api/v1/dev/update-completion"

    split_data = email.split("|")

    payload = json.dumps({
        "email": str(split_data[0]),
        "redeem_code": str(split_data[1]),
        "post_test": int(split_data[2]),
        "pre_test": int(split_data[3]),
        "skill_test": int(split_data[4]),
    })
    headers = {
